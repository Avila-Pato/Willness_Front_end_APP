...
import {
  VINCULOS_STYLE_META,
  VINCULOS_STYLE_ORDER,
} from "@/data/vinculosTestData";
import {
  getCompletedTests,
  markTestCompleted,
  saveTestResult,
} from "@/store/selfTestResults";
...
export default function SelfTestResultScreen() {
  const { testId, result: raw } = useLocalSearchParams<{
    testId: string;
    result: string;
  }>();
  ...
  const testMeta = TEST_CATEGORIES.flatMap((c) => c.tests).find(
    (t) => t.id === testId,
  );
  ...
  useEffect(() => {
    ...
    getCompletedTests().then((list) => {
      ...
      setTimeout(() => {
        setLoading(false);
        if (!wasCompleted) {
          achieveScale.value = withDelay(
            600,
            withSpring(1, { damping: 14, stiffness: 110 }),
          );
          achieveOpacity.value = withDelay(
            600,
    ...
    <SafeAreaView edges={["top", "left", "right"]} style={s.root}>
      ...
      >
        ...
        <View
          style={[
            s.primaryCard,
            {
              ...
              borderColor: primaryMeta.color + "40",
            },
          ]}
        >
          <View
            style={[
              s.primaryBadge,
              { backgroundColor: primaryMeta.color + "20" },
            ]}
          >
            <Text style={[s.primaryBadgeText, { color: primaryMeta.color }]}>
              Perfil principal
            </Text>
          </View>
          <Text style={[s.primaryStyle, { color: primaryMeta.color }]}>
            {primaryMeta.label}
          </Text>
              ...
              return (
                <View key={style} style={s.barRow}>
                  <Text
                    style={[
                      s.barLabel,
                      isPrimary && { fontWeight: "800", color: meta.color },
                    ]}
                  >
                    {meta.short}
                  </Text>
                  ...
                  <Text style={[s.barValue, { color: meta.color }]}>
                    {avg.toFixed(1)}
                  </Text>
                </View>
              );
        ...
        <View style={s.tipCard}>
          <Text style={[s.tipLabel, { color: primaryMeta.color }]}>
            Qué puedes hacer
          </Text>
            ...
            {alreadyCompleted ? (
              <>
                ...
                <Text style={s.achieveDesc}>
                  Este logro ya está en tu perfil. Puedes volver a hacerlo
                  cuando quieras.
                </Text>
              </>
            ) : (
              <>
                ...
                <Text style={s.achieveDesc}>
                  Completaste <Text style={{ fontWeight: "800" }}>{title}</Text>{" "}
                  y ganaste este logro. Encuéntralo en tu perfil.
                </Text>
              </>
            )}
          </Animated.View>
        ...
        <Text style={s.disclaimer}>
          Este test es orientativo y no reemplaza una evaluación clínica. Los
          resultados reflejan tendencias en tus respuestas, no diagnósticos.
        </Text>
      </ScrollView>
